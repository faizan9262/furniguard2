...
const Designers = () => {
  ...
  return (
    <div className="mx-4 sm:mx-[10%] min-h-screen my-8">
      <div className="bg-[#f0fdf4] border border-[#c6f6d5] rounded-xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        <Input
          type="text"
          placeholder="Search by name or type..."
          className="w-full sm:w-[66%] border border-[#2d9b67] bg-white shadow-sm focus:ring-2 focus:ring-[#2d9b67]"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
        />

        <div className="flex flex-col w-full sm:w-auto">
          <Label className="text-sm text-[#1c4532] mb-1 font-medium">
            Filter by Experience
          </Label>
          <Select onValueChange={setExperienceFilter}>
            <SelectTrigger className="w-full sm:w-40 border border-[#2d9b67] bg-white text-[#2d9b67] font-medium shadow-sm focus:ring-2 focus:ring-[#2d9b67]">
              {experienceFilter === "" || experienceFilter === "all"
                ? "All"
                : `${experienceFilter} Years`}
            </SelectTrigger>
            <SelectContent className="bg-white border border-[#2d9b67] text-[#2d9b67] shadow-md">
